...
function CreateGameForm() {
  ...
  const handleCreate = () => {
    if (!user || !selectedSport || !selectedEvent || !selectedPick || !selectedFriend) {
      ...
      <Card className="bg-card/50 backdrop-blur-sm border-white/5 overflow-hidden">
        ...
        <CardContent className="p-6 space-y-10">
          ...
          <div className="space-y-4">
            ...
            {selectedSport ? (
              <div className="space-y-4">
                <Input placeholder="Search..." className="bg-secondary/30 border-white/5" value={searchPickQuery} onChange={(e) => setSearchPickQuery(e.target.value)} />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-[200px] overflow-y-auto">
                  {filteredPicks.map((option) => (
                    <button key={option} onClick={() => setSelectedPick(option)} className={`flex items-center justify-between p-3 rounded-lg border text-xs font-bold ${selectedPick === option ? 'bg-primary/10 border-primary text-primary' : 'bg-secondary/20 border-white/5'}`}>
          ...
          <div className="space-y-6 pt-4 border-t border-white/5">
            <RadioGroup defaultValue="gold" className="grid grid-cols-2 gap-4" onValueChange={setCurrency}>
              <div className={`p-4 rounded-xl border-2 ${currency === 'gold' ? 'border-primary' : 'border-white/5'}`}>
                <RadioGroupItem value="gold" id="gold" /><Label htmlFor="gold" className="ml-2 uppercase text-xs">Gold Coins</Label>
              </div>
              <div className={`p-4 rounded-xl border-2 ${currency === 'sweeps' ? 'border-accent' : 'border-white/5'}`}>
                <RadioGroupItem value="sweeps" id="sweeps" /><Label htmlFor="sweeps" className="ml-2 uppercase text-xs">Sweeps Coins</Label>
              </div>
            </RadioGroup>
            <Select value={fee} onValueChange={setFee}>
              <SelectTrigger className="bg-secondary/30 border-white/5 h-12"><SelectValue /></SelectTrigger>
              <SelectContent><SelectItem value="100">100</SelectItem><SelectItem value="1000">1,000</SelectItem><SelectItem value="5000">5,000</SelectItem></SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-white/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto">
              {filteredFriends.map((friend) => (
                <button key={friend.id} onClick={() => setSelectedFriend(friend.id)} className={`flex items-center gap-3 p-3 rounded-xl border-2 ${selectedFriend === friend.id ? 'bg-primary/10 border-primary' : 'bg-secondary/20 border-white/5'}`}>
        ...
        <CardFooter className="bg-secondary/10 border-t p-6 flex flex-col gap-4">
          <Button onClick={handleCreate} className="w-full h-14 font-headline text-xl font-bold uppercase tracking-widest shadow-xl">Launch Challenge <ArrowRight className="ml-2 h-6 w-6" /></Button>
        </CardFooter>
      </Card>
    </main>
  );
